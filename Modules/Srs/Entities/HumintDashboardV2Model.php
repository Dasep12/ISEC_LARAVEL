...
class HumintDashboardV2Model extends Model
{
                ...
                SELECT TOP 10 ris.id, ris.title, ISNULL(tis.total, 0) total
                    ...
                    LEFT JOIN ( 
                        select count(1) total, stis.risk_id
                            ...
                            $q .= " AND stis.area_id in (SELECT ajp.area_id from isecurity.dbo.admisec_area_users aau
                                INNER JOIN dbo.admisec_area_join_plant ajp ON aau.plant_id=ajp.plant_id 
                            WHERE aau.npk=$npk)";
                ...
                SELECT m.MonthNum month_num ,count(t.id) total
                    ...
                    LEFT OUTER JOIN (
                        select str.id ,str.event_date ,str.area_id
                            ...
                            $q .= " AND str.area_id in (SELECT ajp.area_id from isecurity.dbo.admisec_area_users aau
                                    INNER JOIN dbo.admisec_area_join_plant ajp ON aau.plant_id=ajp.plant_id 
                                WHERE aau.npk=$npk)";
                            if(AuthHelper::is_building_manager()) {
            ...
            SELECT rsu.id ,rsu.title ,ISNULL(t.total,0) total
                ...
                LEFT OUTER JOIN (
                    SELECT count(1) total ,atr.".$sub_name."
                        ...
                        $q .= " AND atr.area_id in (SELECT ajp.area_id from isecurity.dbo.admisec_area_users aau
                                INNER JOIN dbo.admisec_area_join_plant ajp ON aau.plant_id=ajp.plant_id 
                            WHERE aau.npk=$npk)";
        ...
        if($req->isMethod('post'))
        {
            ...
            $q = "
                ...
                SELECT m.MonthNum month_num, count(t.id) total
                    ...
                    $q .= " AND t.area_id in (SELECT ajp.area_id from isecurity.dbo.admisec_area_users aau
                            INNER JOIN dbo.admisec_area_join_plant ajp ON aau.plant_id=ajp.plant_id 
                        WHERE aau.npk=$npk)";
    ...
    public static function grap_trans_area($req)
    {
        ...
        if($req->isMethod('post'))
        {
            ...
            // {
                ...
                $q .= " AND aar.id in (SELECT ajp.area_id from isecurity.dbo.admisec_area_users aau
                        INNER JOIN dbo.admisec_area_join_plant ajp ON aau.plant_id=ajp.plant_id 
                    WHERE aau.npk=$npk)";
            // }
            if(!empty($area)) $q .= ' AND ';
        ...
        if($req->isMethod('post'))
        {
            ...
            $q = "
                SELECT count(1) total
                    ...
                    $q .= " AND tis.area_id in (SELECT ajp.area_id from isecurity.dbo.admisec_area_users aau
                            INNER JOIN dbo.admisec_area_join_plant ajp ON aau.plant_id=ajp.plant_id 
                        WHERE aau.npk=$npk)";
    ...
    public static function grap_srs($req)
    {
        ...
        $q .= " AND tio.area_id in (SELECT ajp.area_id from isecurity.dbo.admisec_area_users aau
                INNER JOIN dbo.admisec_area_join_plant ajp ON aau.plant_id=ajp.plant_id 
            WHERE aau.npk=$npk)";
    ...
    public static function grap_soi($req)
    {
        ...
        // if(AuthHelper::is_building_manager()) {
            ...
            $q .= " AND soi.area_id in (SELECT ajp.area_id from isecurity.dbo.admisec_area_users aau
                    INNER JOIN dbo.admisec_area_join_plant ajp ON aau.plant_id=ajp.plant_id 
                WHERE aau.npk=$npk)";
        // }
        if(!empty($area) || !empty($year) || !empty($month)) $q .= ' AND ';
